...
def compute_pk_with_cv(cosmo, z_values, z_values_all, nz_values, delta_A0=.05, variability=True):
    ...
    for a in a_all:
        print(f"calculating for z = {1/a -1:.2f}, the final z: {1/a_all[-1] - 1:.2f}", end="\r")
        ...
        if variability:
            tk3d = halomod_Tk3D_cNG(cosmo.cosmoccl, hmc, profile,
                                    lk_arr=np.log(k), a_arr=np.array([a]),
                                    use_log=False)
            Tmat = tk3d(k, a)

            cov_total = compute_total_covariance(tk3d, k, a, P_mean, V_eff, delta_A0=delta_A0)
    ...
    if variability:
        ...
        pk_dict = {\
            z: P_samples_all[i,:]\
            for i, z in enumerate(z_values_all)\
        }
        return pk_dict
    else:
...
def compute_total_covariance(tk3d, k, a, pk_mean, volume, delta_A0=1.,
    ...
    # Total covariance
    cov_total = cov_gauss + 0.3*cov_ng + (0.35 *cov_ssc)
    # cov_to0al = cov_ssc
    # plt.figure()
