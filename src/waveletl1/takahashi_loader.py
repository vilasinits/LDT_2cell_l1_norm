...
class TakahashiLoader:
    def __init__(
        self,
        data_path="/feynman/work/dap/lcs/vt272285/data/sim_data",
        takahashi_data_path=None,
    ):
    ...
    def get_beam(self, theta, lmax):
        def top_hat(b, radius):
            return np.where(
                abs(b) <= radius, 1 / (np.cos(radius) - 1) / (-2 * np.pi), 0
            )
    ...
    def smooth_map(self, args):
        ...
        kappa_smooth = hp.sphtfunc.alm2map(
            hp.sphtfunc.almxfl(almkappa, beam), nside_takahashi
        )
        return kappa_smooth
    ...
    def run_loader(self, theta1, nside, zs, nprocess):
        ...
        takahashi_file = (
            self.takahashi_data_path
            + "allskymap_nres12r000.zs"
            + str(tk_fn[index])
            + ".mag.dat"
        )
        print("the takahashi file being loaded is: ", takahashi_file)
        var, kmap1, kmap2 = self.load_takahashi_file(
            takahashi_file, theta1, nside, nprocess
        )
